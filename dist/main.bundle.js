...
var AppComponent = (function () {
    function AppComponent(httpService) {
        ...
        this.citys = []; // Все города полученные от сервера
        this.okSearch = true; // Ответ сервера
        this.wait = true; // Ожидание ответа от сервера
    ...
    AppComponent.prototype.ngOnInit = function () {
        ...
        this.filteredCitys = this.myControl.valueChanges
            .startWith(null)
            .map(function (val) { return val ? _this.filter(val).slice(0, 4) : []; }); // Фильтрация и сокращение обьекта до 5 элементов
    };
    AppComponent.prototype.getDataServer = function () {
        var _this = this;
        this.httpService.getData() //Получаем данные с сервера
            .subscribe(function (data) {
            var tmp = {}; // Обьект для запоминания повторяющихся городов
            _this.citys = data.json().filter(function (city) {
                return city.City in tmp ? 0 : tmp[city.City] = 1;
            });
            _this.okSearch = true; // Статус ответа
        }, function (err) {
            if (!err.ok) {
                _this.okSearch = err.ok;
            }
        }, function () {
            setTimeout(function () { return _this.wait = false; }, 1000);
        });
    };
    AppComponent.prototype.filter = function (val) {
        if (this.myControl.value.length >= 1) {
            var filteredCitysAll = this.citys.filter(function (city) {
                return city.City.toLowerCase().indexOf(val.toLowerCase()) === 0;
            });
